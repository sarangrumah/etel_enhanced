...
class EvaluatorController extends Controller
{
    ...
    public function ulo()
    {
        ...
        $izin = Izin::select('*')
            ->join('tb_trx_disposisi_evaluator as d', 'd.id_izin', '=', 'vw_list_izin.id_izin')
            ->where('d.id_disposisi_user', '=', $id_user_session)
            ->whereIn('vw_list_izin.status_checklist', [601])
            // ->where('vw_list_izin.id_master_izin_parent', '=', $id_departemen_user)
            // ->take($limit_db);
            ->get();
        // $izin = $izin->where('id_master_izin_parent','=',$id_departemen_user)->distinct('vw_list_izin.id_izin');
        ...
        $jenis_izin = 'Izin Penyelenggaraan Jaringan Telekomunikasi';
        // dd($izin);
        return view('layouts.backend.evaluator.dashboard-ulo', ['date_reformat' => $date_reformat, 'ulo' => $ulo, 'izin' => $izin, 'jenis_izin' => $jenis_izin, 'data' => $data]);
    }
    ...
    public function evaluasiUloPost($id, $urut, Request $request)
    {
        ...
        if ($request->file('dok_peta_jaringan') != null) {
            ...
            if ($update_bukti_perangkat > 0) {
                // Update was successful
                echo "Update successful. Rows affected: " . $update_bukti_perangkat;
            } else {
    ...
    public function ulangPenomoran($id)
    {
        // dd($id);
        $id_user_session = Session::get('id_user');
        $id_departemen_user = Session::get('id_departemen');
        ...
        // $id = $penomoran_alokasi->id;
        // dd($penomoran_alokasi->id_mst_kode_akses);
        return view('layouts.backend.evaluator.evaluasi-penetapan-ulangpenomoran', [
            'date_reformat' => $date_reformat, 'id' => $id,
            'penomoran' => $penomoran_alokasi,
            'jenis_perseroan' => $jenis_perseroan,
            'instansi_npt' => $instansi_npt,
            'instansi_bh' => $instansi_bh,
            'provinsi' => $provinsi
        ]);
    }
    ...
    public function savepenetapanulangpenomoran(Request $req)
    {
        // dd($req, $req->input('btn_draft'));

        if($req->action=='draft'){
            $penomoran = DB::table('vw_alokasi_penomoran_rev as t')
            ->where('t.id', '=', $req->id)->first();
            $alamat = DB::table('vw_detail_loc')->where('id','=',$req->vKelurahan)->first();
            $alamat_full = $req->alamat_perseroan . ', ' . $alamat->detail_loc . ' ' . $req->kodepos;
            $jenis_layanan = DB::table('tb_mst_izinlayanan')->where('kode_izin','=',$req->jenislayanan)->first();
            // dd($jenis_layanan->short_name_nomor_html);
            $status = 'Penetapan';

            $date_reformat = new DateHelper();
            $penomoran->nib = $req->nib;
            $penomoran->alamat_perseroan = $alamat_full;
            $penomoran->nama_perseroan = $req->nama_perseroan;
            $penomoran->oss_kode = $req->alokasi_jenispengguna;
            $penomoran->dasarpenetapan = $req->dasarpenetapan;
            $penomoran->jenis_institusi = $req->vJenisInstansi;
            if(isset($id_izin)){
                $penomoran->id_izin = $id_izin;
            }else{
                $penomoran->id_izin = '';
            }
            if(isset($jenis_layanan)){
                $penomoran->jenislayanan = $jenis_layanan->short_name_nomor_html;
            }
            // $penomoran = $penomoran->toArray();
            ...
            if($req->alokasi_jenispengguna == "01"){
                    $pdf = PDF::loadView('layouts.backend.sk.draft-penomoranulang-nib',
                    ['data'=>$penomoran,'datanib'=>$penomoran,'penomoran'=>$penomoran,'date_reformat'=>$date_reformat]
                    );//->setPaper('a4', 'portrait');
            }else{
                    $pdf = PDF::loadView('layouts.backend.sk.draft-penomoranulang',
                    ['data'=>$penomoran,'datanib'=>$penomoran,'date_reformat'=>$date_reformat]
                    );//->setPaper('a4', 'portrait');
            }

            return $pdf->stream();
        }elseif($req->action=='disposisi'){
            // dd($req->all());
            // try{
                DB::beginTransaction();
                $log = new LogHelper();
                $jenis_permohonan = "Penetapan Ulang Penomoran Telekomunikasi";
                $catatan_hasil_evaluasi = $req->get('catatan_hasil_evaluasi');
                // dd($req->all());
                $status_penomoran = 901;
                if ($file1 = $req->file('berkas_tambahan')) {
                    $filename1 = "DataDukungPenetapan-" . time() . '.' . $file1->extension();
                    $path1 = $file1->storeAs('public/penambahan/penomoran', $filename1);
                    $name1 = $file1->getClientOriginalName();
                    $path1 = str_replace('public/', 'storage/', $path1);
                    // $penambahan_flag = true;
                ...
                if ($file2 = $req->file('berkas_nib')) {
                    $filename2 = "DataNIB-" . time() . '.' . $file2->extension();
                ...
                $insert = new Izinoss([
                    // 'oss_id' => $oss_id,
                    'id_proyek' => $id_jeniskodeakses->short_name,
                    'nib' => isset($req->nib) ? $req->nib : null,
                    'id_izin' => $maxId,
                    'jenis_izin' => $jenis_izin,
                    // 'kd_daerah' => $oss_nib->perseroan_daerah_id,
                    'kd_izin' => isset($req->jenislayanan) ? $req->jenislayanan : null,
                    'status_checklist' => '902',
                    'submitted_at' => date('Y-m-d H:i:s')
                ]);
                ...
                if ($insert_trxkodeakses->save()) {
                    $update_penomoran_penetapanulang = DB::table('tb_trx_penomoran_penetapanulang')
                    ->insert([
                    'id_izin' => $maxId,
                    'id_mst_kode_akses' => $req->id,
                    'jenis_layanan' => $req->jenislayanan,
                    'nib' => $req->nib,
                    'nama_institusi' => $req->nama_perseroan,
                    'jenis_institusi' => $req->vJenisInstansi,
                    'alamat_institusi' => $req->alamat_perseroan,
                    'id_mst_kelurahan' => $req->vKelurahan,
                    'kodepos' => $req->kodepos,
                    'email_institusi' => $req->email_perusahaan,
                    'dasar_penetapanulang' => $req->dasarpenetapan,
                    'berkas_nib' => $path2,
                    'berkas_tambahan' => $path1,
                    ...
                    return back()->with('error', $msg_failed);
                }
    ...
    public function koreksi_komitmen(Request $request)
    {
        $date_reformat = new DateHelper();
        $limit_db = Config::get('app.admin.limit');
        $id_departemen_user = Session::get('id_departemen');
        ...
        $komitmen_list = Izin::where('status_checklist', '=', 10)->where('id_master_izin','=',$id_jabatan);

        if ($komitmen_list->count() > 0) { //handle paginate error division by zero
            $komitmen_list = $komitmen_list->paginate($limit_db);
        } else {
            $komitmen_list = $komitmen_list->get();
        }
        $paginate = $komitmen_list;
        $komitmen_list = $komitmen_list->toArray();

        $countpenomoran = IzinHelper::countPenomoran(904);

        // dd($komitmen_list);

        return view('layouts.backend.evaluator.dashboard-penyesuaian', ['date_reformat' => $date_reformat, 'paginate' => $paginate, 'komitmen_list' => $komitmen_list, 'countpenomoran' => $countpenomoran]);
    }

    public function koreksi_komitmen_id(Request $request)
    {
        $date_reformat = new DateHelper();
        $limit_db = Config::get('app.admin.limit');
        $id_departemen_user = Session::get('id_departemen');
        ...
        $nib = DB::table('tb_oss_nib')->select('oss_id')->where('nib','=',$izin->nib)->first();

        $common = new CommonHelper();
        $detailNib = $common->get_detail_nib_by_oss($nib->oss_id);
        // dd($detailNib);
        ...
        return view('layouts.backend.evaluator.koreksi-komitmen', compact('id_izin', 'izin', 'izin2', 'detailNib', 'penanggungjawab', 'datasyarattable', 'datasyaratpdf', 'map_izin'));
    }

    public function koreksikomitmenpost($id,Request $req)
    {
        // dd($req);
        DB::beginTransaction();
        // try {
        ...
        // if ($izin) {
            ...
            $izinToLog['status_checklist'] = '45';
            $izinToLog['created_by'] = Session::get('id_user');
            $izinToLog['created_name'] = Session::get('nama');
            // dd($izinToLog);
            $insertIzinLog = Izinlog::create($izinToLog);

            DB::commit();
            return redirect()->route('admin.evaluator.koreksikomitmen')->with('message', 'Komitmen telah berhasil diperbaharui');
        // }
        // } catch (\Throwable $th) {
        //     DB::rollback();
        //     // throw ValidationException::withMessages(['message' => 'Gagal']);
