...
@section('content')
	...
	<div class="card">
		...
		@endif
		<div class="card-body">
			<table class="table text-nowrap datatable-basic" id="index">
				<thead>
					<tr>
				...
				<tbody>
					@foreach ($izin as $item)
						<tr>
								...
								<div class="d-flex align-items-center">
									<div>
										...
										@if (isset($item['kbli']))
											...
											@endif
										@else
											<div class="text-muted font-size-sm">{{ $item['jenis_izin'] }}
											</div>
										@endif
							...
							<td class="text-center"><span class="badge badge-success-100 text-success">{{ $item->batas_verifikasi }}</span>
							</td>
									...
									<div class="dropdown-menu dropdown-menu-right">
										@if ($item['jenis_perizinan'] != 'K03')
											...
											@if ($item['status_checklist'] == '10')
												<a href="{{ url('komitmen/penyesuaian') . '/' . $item->id_izin }}" class="dropdown-item" target="_blank"><i
														class="icon-history"></i>
										...
										@else
											@if ($item['layanan_penomoran'] == '1')
												<a href="{{ url('/penomoran/') . '/' . $item->id_izin }}" class="dropdown-item"><i
														class="icon-file-upload"></i> Permohonan
													Kode
													Akses</a>
											@else
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="mb-4">
							<div class="mb-3">
								<p>Perizinan</p>
								<select name="perizinan" class="form-control" required>
									<option value="">Silakan Pilih..</option>
							...
							<div id="KBLIjasa" hidden>
								<div class="mb-3">
									<p>KBLI</p>
									<select name="kbli" class="form-control" required>
										<option value="">Silakan Pilih..</option>
...
@section('custom-js')
	<script type="text/javascript">
		$(document).ready(function() {
			if (!$.fn.dataTable.isDataTable('#index')) {
				$('#index').DataTable({
					columnDefs: [{
						targets: [1], // Hide columns with indices 0 and 4
						visible: false
					}]
				});
			}
			...
			// // Destroy DataTables instance before reinitializing (if needed)
			...
			// // Example of reinitializing DataTables after dynamic content change
			// function updateTableContent() {
			// 	// Code to update table content dynamically
			...
			// 	$('#table').DataTable({
			// 		// DataTable initialization options for updated content
			// 	});
			// }
