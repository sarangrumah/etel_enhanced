                    ...
                    @if (isset($d['isdeleted']))
                        @if ($d['isdeleted'] == '1')
                            ...
                            <tr class="rolloutplan-item">
                                ...
                                <td style="width: 5%;">
                                    <input type="number" min="1" value="{{ $d['jumlah_node'] }}"
                                        {{ $is_editable ? '' : 'readonly' }} class="form-control jumlah_node"
                                        name="rolloutplan[{{ $key }}][jumlah_node]" required />
                                
                                </td>
                                <td style="width: 15%;">
                                    <select {{ $is_editable ? '' : 'readonly' }}
                                        class="form-control lokasi_cable_landing_station select w-100"
                                        name="rolloutplan[{{ $key }}][lokasi_node][]" multiple="multiple" required />
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ in_array($city->id, $d['lokasi_node']) ? 'selected' : '' }}>{{ $city->name }}
                                        </option>
                                    @endforeach
                                    </select>
                                </td>
                                <td style="width: 15%;">
                                    <select {{ $is_editable ? '' : 'readonly' }}
                                        class="form-control lokasi_cable_landing_station select w-100"
                                        name="rolloutplan[{{ $key }}][cakupan_wilayah_layanan][]" multiple="multiple"
                                        required>
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                ...
                                <td style="width: 10%;">
                                    ...
                                    <input type="number" min="1" value="{{ $d['panjang_rute_kabel_fiber_optik'] }}"
                                        ...
                                        class="form-control panjang_rute_kabel_fiber_optik"
                                ...
                                @if ($needcorrection == 1)
                                    <td style="width: 8%;">
                                            <div>
                                                <select name="rolloutplan[{{ $key }}][isdeleted]"
                                                    id="" class="form-control isdeleted_cakupanwilayahtelsus_mtk"
                                                    required>
                                                    <option
                                                        name="rolloutplan[{{ $key }}][isdeleted]"
            ...
            @else
                @for ($i = 0; $i < 5; $i++)
                    <tr class="rolloutplan-item">
                        ...
                        <td style="width: 15%;">
                            <select class="form-control select w-100"
                                name="rolloutplan[{{ $i }}][lokasi_node][]" 
                                multiple="multiple" 
                                required>
                            @foreach ($cities as $city)
                        ...
                        <td style="width: 15%;">
                            <select class="form-control select w-100"
                                name="rolloutplan[{{ $i }}][cakupan_wilayah_layanan][]" multiple="multiple" placeholder="Pilih Kota"
                                required>
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}">{{ $city->name }}</option>
                            @endforeach
...
<script type="text/javascript">
            ...
            $('#add-RolloutPlan').on('click', function() {
                ...
                const inputRow =
                    ...
                    this.totalRolloutPlan + `][jumlah_node]" required />
						...
						<td>
							<input min="1" oninput="validity.valid||(value='');" type="number" class="form-control jumlah_kabel_fiber_optik" name="rolloutplan[` + this.totalRolloutPlan + `][jumlah_kabel_fiber_optik]" required />
						</td>
						<td>
							<input class="form-control kapasitas_bandwidth" name="rolloutplan[` + this.totalRolloutPlan + `][kapasitas_bandwidth]" required />
						</td>
						<td>
							<input min="1" oninput="validity.valid||(value='');" type="number" class="form-control panjang_rute_kabel_fiber_optik" name="rolloutplan[` + this.totalRolloutPlan + `][panjang_rute_kabel_fiber_optik]" required />
						</td>
						<td style="width: 5%;">
							<button
    ...
    function onDeleteRolloutPlanItem(e) {
        ...
        $('.rolloutplan-item').each(function(index, element) {
            ...
            panjang_rute_kabel_fiber_optik.attr('name', 'rolloutplan[' + index + '][panjang_rute_kabel_fiber_optik]');
        });
    }
</script>