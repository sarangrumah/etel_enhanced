	...
	<form action="{{ url('/ip/registerpt') }}" method="post" enctype="multipart/form-data">
		...
		<fieldset>
			...
			{{-- <div class="form-group row">
				<div class="col-lg-12">
					<label class="col-lg-8 col-form-label font-weight-semibold" id="lbl_kbli">KBLI yang dimiliki</label>
					<div class="col-lg-6" id="PilihKBLI">
						<table class="table table-custom table-sm">
							<tbody id="kbli-lists">
								<tr class="kbli-item">
									<td style="width: 10%;">
										<input type="text" name="no_urut" id="no_urut" value = '1'
											class="form-control h-auto justify-content-center" readonly>
									</td>
									<td style="width: 60%;">
									...
									<td style="width: 10%;">
										<div class="row">
											<button type="button" class="btn btn-success add-kbli" id="add-kbli"><i class="icon-plus3"></i></button>
										</div>
									</td>
								</tr>
							</tbody>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		const kbliLists = document.getElementById('kbli-lists');

		document.getElementById('add-kbli').addEventListener('click', function() {
			const newRow = kbliLists.querySelector('.kbli-item').cloneNode(true);
			newRow.querySelector('select').selectedIndex = 0;
			newRow.querySelector('.remove-kbli').addEventListener('click', function() {
				kbliLists.removeChild(newRow);
			});
			kbliLists.appendChild(newRow);
		});

		kbliLists.querySelector('.remove-kbli').addEventListener('click', function() {
			kbliLists.querySelector('.kbli-item').remove();
		});
	});
</script>
