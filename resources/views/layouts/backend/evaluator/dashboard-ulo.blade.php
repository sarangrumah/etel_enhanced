	...
	<div class="row">
		<div class="col-lg">
			...
			<div class="card bg-primary text-white">
				<div class="card-body">
					<div class="d-flex">
						<h3 class="font-weight-semibold mb-0">{{ $data }}</h3>
	...
	<div>
				...
				<table class="table text-nowrap datatable-button-init-basic" id="table">
					<thead>
						<tr>
							...
							<th class="text-center" style="width: 5%;" hidden></th>
								...
								<tr>
									<td>
										<div class="d-flex align-items-center">
											<div>
												<a href="{{ route('admin.evaluator.evaluasi-ulo', [$ulos['id_izin'], $ulos['id']]) }}"
													class="text-body font-weight-semibold">{{ $ulos['id_izin'] }}</a>
												<div class="text-muted font-size-sm"> {{ strtoupper($ulos['nama_perseroan']) }}
												</div>
												<div class="text-muted font-size-sm">{{ $ulos['jenis_izin'] }}</div>
												<div class="text-muted font-size-sm">{{ $ulos['kbli'] }} - {{ $ulos['kbli_name'] }}
												</div>
									...
									@else
										<td class="text-center"> {{ $date_reformat->dateday_lang_reformat_long($ulos['tgl_submit']) }}
										</td>
									@endif
									<td class="text-center" hidden>3 Hari</td>
									@if ($ulos['tgl_pengajuan_ulo'] == null)
										<td class="text-center"> - </td>
									@else
										<td class="text-center">
											{{ $date_reformat->dateday_lang_reformat_long($ulos['tgl_pengajuan_ulo']) }}</td>
									@endif
									@if ($ulos['tipe_ulo'] == '1')
										<td class="text-center">-</td>
									@else
									...
									@endif
									<td class="text-center">
										<span class="badge badge-success-100 text-success">{{ $ulos['name_status_bo'] }}</span>
									</td>
									<td class="text-center">
										<div class="dropdown">
											<a href="#" class="btn btn-outline-light btn-icon btn-sm text-body border-transparent rounded-pill"
											...
											<div class="dropdown-menu dropdown-menu-right">
												@if ($ulos['status_survey'] === 0 && $ulos['status_ulo'] !== '901')
													<a href="{{ route('admin.evaluator.kirim-survey', $ulos['id_izin']) }}" class="dropdown-item"><i
															class="icon-stats-dots"></i> Kirim Survey IKM &
														IIPP</a>
												@else
													<a href="{{ route('admin.evaluator.tanggal-evaluasi-ulo', [$ulos['id_izin'], $ulos['id']]) }}"
														class="dropdown-item"><i class="icon-calendar"></i> Ubah Tanggal Pelaksanaan
														ULO</a>
													<a href="{{ route('admin.evaluator.evaluasi-ulo', [$ulos['id_izin'], $ulos['id']]) }}"
														class="dropdown-item"><i class="icon-pencil"></i> Evaluasi Perizinan</a>
		...
		<div class="card">
			<div class="card-header">
				<div class="row">
					<div class="col-lg-8">
						<h6 class="card-title font-weight-semibold py-3">Daftar Permohonan Izin Penyelenggaraan Telekomunikasi
							Khusus
							...
							@foreach ($izin as $izins)
								<tr>
									<td>
										<div class="d-flex align-items-center">
											<div>
												<a href="{{ route('admin.evaluator.evaluasi', $izins['id_izin']) }}"
													class="text-body font-weight-semibold">{{ $izins['id_izin'] }}</a>
												<div class="text-muted font-size-sm">{{ strtoupper($izins['nama_perseroan']) }}
												</div>
												<div class="text-muted font-size-sm">{{ $izins['jenis_izin'] }}</div>
												@if ($izins['status_checklist'] == 801)
												@else
									...
									<td class="text-center"><span
										...
										Hari</td>
									<td class="text-center">
										@if ($izins['status_penyesuaian'] == '20')
									...
									<td class="text-center">
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-right">
												@if ($izins['status_penyesuaian'] == '20')
													<a href="{{ route('admin.evaluator.evaluasi.penyesuaian', $izins['id_izin']) }}" class="dropdown-item"><i
															class="icon-pencil"></i> Evaluasi Perubahan
														Komitmen</a>
												@else
													<a href="{{ route('admin.evaluator.evaluasi', $izins['id_izin']) }}" class="dropdown-item"><i
															class="icon-pencil"></i> Evaluasi
														Perizinan</a>
