...
<p style="text-left"> Permohonan Penomoran Izin Penyelenggaraan Telekomunikasi Saudara dengan detail permohonan sebagai
	berikut telah disetujui dan Surat Penetapan Penomoran telah diterbitkan.</p>
...
<table>
	...
	<tr>
		...
		<td>: @if (isset($email_data['nib']['nama_perseroan']))
				{{ $email_data['nib']['nama_perseroan'] }}
			@else
				{{ $email_data['nib']['nama_institusi_ulang'] }}
			@endif
			{{-- {{ isset($email_data['nib']['nama_perseroan']) ? $email_data['nib']['nama_perseroan'] : '-' }} --}}
		</td>
	</tr>
	@if (substr($email_data['izin']['id_izin'], 0, 3) != 'TKI' &&
			substr($email_data['izin']['id_izin'], 0, 3) != 'NOM' &&
			substr($email_data['izin']['id_izin'], 0, 3) != 'NEV')
		<tr>
			<td>Nomor Izin Berusaha (NIB) </td>
			<td>: @if (isset($email_data['izin']['nib']))
					{{ $email_data['izin']['nib'] }}
				@elseif (isset($email_data['izin']['nib_ulang']))
					{{ $email_data['izin']['nib_ulang'] }}
				@else
					{{ '-' }}
				@endif
				{{-- {{ isset($email_data['izin']['nib']) ? $email_data['izin']['nib'] : '-' }} --}}
			</td>
		</tr>
	@elseif(substr($email_data['izin']['id_izin'], 0, 3) == 'NEV')
		@if (isset($email_data['izin']['nib_ulang']))
			<tr>
				<td>Nomor Izin Berusaha (NIB) </td>
				<td>: {{ $email_data['izin']['nib_ulang'] }}
				</td>
			</tr>
		@endif
	@endif
	...
	@if (isset($email_data['izin']['full_kbli']))
		<tr>
			<td>KBLI </td>
